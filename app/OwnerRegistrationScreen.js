          ...
          <KeyboardAvoidingView style={{ flex: 1 }} behavior="padding">
            <View style={{ flex: 1, padding: 20 }}>
              ...
              <View style={styles.card}>
                ...
                <Text style={styles.cardText}>
                  Check tenant bookings & requests
                </Text>
              </View>

              <View style={styles.card}>
                <Text style={styles.cardTitle}>Profile</Text>
                <Text style={styles.cardText}>Update your account details</Text>
              </View>
              ...
              {/* <TouchableOpacity
                ...
                onPress={() => setScreen("ownerLogin")} // Opens OwnerLoginScreen
              >
                <Text
                  style={{
                    color: "white",
    ...
    <SafeAreaView style={{ flex: 1 }} edges={["left", "right", "bottom"]}>
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={{ flex: 1 }}>
          ...
          {screen === "ownerLogin" && (
            <OwnerLoginScreen setScreen={setScreen} />
          )}
                            ...
                            <View style={styles.mapWrap}>
                              {Platform.OS === "web" ? (
                                <Image
                                  ...
                                  source={{
                                    uri: staticMapUrl(
                                      (mapRegion || DEFAULT_REGION).latitude,
                                      (mapRegion || DEFAULT_REGION).longitude,
                                    ),
                                  }}
                                />
                              ) : (
                                <MapView
                                  ...
                                  mapType={mapType}
                                  showsUserLocation={false} // do not auto-center on user's current location
                          ...
                          {mapRegion && (
                            ...
                            >
                              <TouchableOpacity
                                ...
                                onPress={async () => {
                                  // Removed automatic current-location logic; map already shows the location entered by user
                                }}
                              >
...
function Step3({ form }) {
  ...
  const applyBatchRooms = () => {
    ...
    selectedFloors.forEach((idx) => {
      updated[idx].rooms = Array.from({ length: num }, (_, i) => ({
        roomNo: i + 1,
        beds: 1,
      }));
    });
  ...
  const generateRoomsForFloor = () => {
    ...
    const updated = [...floors];
    updated[selectedFloor].rooms = Array.from({ length: num }, (_, i) => ({
      roomNo: i + 1,
      beds: 1,
    }));
    setFloors(updated);
            ...
            <ScrollView contentContainerStyle={step3Styles.gridContainer}>
              {floors.map((floor, index) => (
                ...
                >
                  ...
                  <Text
                    style={[
                      step3Styles.cardTitle,
                      selectedFloors.includes(index) && { color: "#2F80ED" },
                    ]}
                  >
                    Floor {floor.floorNo}
                  </Text>
            ...
            {roomsOpen && selectedFloor !== null && (
              ...
              >
                ...
                <View style={step3Styles.row}>
                  ...
                  <TouchableOpacity
                    style={[step3Styles.setBtn, { backgroundColor: "#22C55E" }]}
...
function ApartmentLayout() {
  ...
  const applyBatchFlats = () => {
    ...
    const updated = [...floors];
    selectedFloors.forEach((idx) => {
      updated[idx].flats = Array.from({ length: num }, (_, i) => ({
        flatNo: i + 1,
        bhk: 1,
      }));
    });
  ...
  const generateFlatsForFloor = () => {
    ...
    if (isNaN(num) || num <= 0 || selectedFloor === null) return;
    const updated = [...floors];
    updated[selectedFloor].flats = Array.from({ length: num }, (_, i) => ({
      flatNo: i + 1,
      bhk: 1,
    }));
    setFloors(updated);
